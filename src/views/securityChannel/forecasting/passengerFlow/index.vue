<template>
  <div class="user-box">
    ...
    <div class="add-user">
      <div>
        <el-button type="primary" size="small" @click="searchData('formDeskEdit')">刷 新</el-button>
    ...
    <el-table :data="tableData" highlight-current-row border stripe>
      ...
      <el-table-column fixed="right" label="操作" min-width="100">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" @click="editData(scope.$index, scope.row)">修 改</el-button>
...
<script>
import { initData, removeData } from '@/api/passengerFlow1'
export default {
  name: 'PassengerFlow',
  filters: {
  ...
  methods: {
    ...
    addData() {
      this.$router.push({ path: '/dispatching/securityChannel/operationPage' })
    },
    editData(index, row) {
      this.$router.push({
        path: '/dispatching/securityChannel/operationPage',
    ...
    openNext(index, row) {
      this.$router.push({
        path: '/dispatching/securityChannel/detailsPage',
        query: { id: row.id }
      })
    }
    // select() {
    ...
    //     console.log(this.carriersOptions, 456)
