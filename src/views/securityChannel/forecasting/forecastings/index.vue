<template>
  <div class="warp">
    <div class="search">
      ...
      <!-- <el-button type="info" size="small" @click="jumpTest">跳转测试</el-button> -->
    </div>
      ...
      <el-card :body-style="{ padding: '0px' }" class="box-card">
        ...
        <div class="seach-content">
          <div class="search-content-item">
            <div class="item1">安检通道处理速率(人/小时)</div>
            <div class="item2">
          ...
          <div class="search-content-item">
            <div class="item1">安检通道最小开放时间(分钟)</div>
            <div class="item2">
          ...
          <div class="search-content-item itemLast">
            <div class="item1">安检通道列表(共{{deskList.length}}个)</div>
            <div class="item2" style="color: #87a054">
      ...
      <div class="chart-search">
        ...
        <span style="display: inline-block">
          <el-popover placement="top-start" width="200" trigger="hover">
            <p style="font-size: 12px">按周显示: 以周为单位，默认查询下周预测结果</p>
            <p style="font-size: 12px">按日显示: 查询指定日期区间的预测结果</p>
            <div slot="reference">
    ...
    <el-dialog v-dialogDrag :title="lineChartDataBig.day" width="70%" :visible.sync="dialogTableVisible" :close="closeImport" center>
      <p style="text-align: right; color: #cf9225">
        安检服务标准完成率：{{ lineChartDataBig.satisfactionPercentage }}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;出港航班数：{{ lineChartDataBig.totalNumber
        }}<el-button type="info" size="mini" style="margin-left: 5px" @click="deskListCLick(lineChartDataBig.deskList)">安检通道开放列表</el-button>
      </p>
    ...
    <el-dialog v-dialogDrag :title="lineChartDataBig.day" width="50%" :visible.sync="deskTableVisible" center>
      <el-table :data="tableData" border :header-row-style="{ 'background-color': 'rgb(10, 20, 47)' }" height="300" :row-style="{ 'background-color': '#232c47' }" style="width: 100%; color: white">
        <el-table-column prop="deskCode" label="安检通道编号" />
...
import { queryList, detailList, Prediction, reCalculate } from '@/apiNew/securityChannel'
export default {
  name: 'SecurityForecasting',
  ...
  methods: {
    ...
    jumpTest() {
      this.$router.push({ path: '/dispatching/securityChannel/securityDispatching' })
    },
    preMonSun() {
      var Stamp
      Stamp = new Date()
    ...
    paraModify() {
      ...
      this.$router.push({ path: '/dispatching/securityChannel/addPage', query: { id: id, businessDomainId: this.businessDomainId } })
    },
    Prediction() {
              ...
              if (response.data.data) {
                if (response.data.data.points) {
                  ...
                  Object.keys(response.data.data.points).forEach(function (cur2) {
                    ...
                    object.datas.expectedDataName2 = '安检通道开放数'
    ...
    openBaseData2() {
      this.$router.push({ path: '/dispatching/securityChannel/parameter', query: { businessDomainId: this.businessDomainId } })
    },
    showEchart(items) {
      // 点击显示大图
      console.log(items, 5656)
      ...
      this.lineChartDataBig.legendData = ['预计到达旅客', '安检通道开放数', '可通过人数', '预计排队时间']
