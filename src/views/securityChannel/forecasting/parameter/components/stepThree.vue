<template>
  <div>
    <el-card class="box-card">
      ...
      <el-form :inline="true" :model="listQuery" class="add-user" size="small">
        <!-- <el-form-item label="机场">
          <el-select v-model="listQuery.airportId" filterable :disabled="disabled" clearable style="width: 140px">
    ...
    <!-- 弹框 -->
    <el-dialog v-dialogDrag :title="lineChartDataBig.bigDate" center :visible.sync="dialogTableVisible" :before-close="closeImport">
      <!-- <h3 style="margin-top: 0;">{{ lineChartDataBig.bigDate }}</h3> -->
...
<script>
import { flightPlan, search, mergerResult } from '@/apiNew/securityChannel'
...
export default {
  ...
  methods: {
    ...
    mergerResult(val) {
      ...
      mergerResult(data).then((response) => {
        ...
        if (response.data.code === '200') {
          ...
          data.forEach((element) => {
            const dataX = []
            const dataY = []
            var myDate = new Date(Date.parse(element.date.replace(/-/g, ' / ')))
            var bigDate = element.date + '/' + weekDay[myDate.getDay()]
