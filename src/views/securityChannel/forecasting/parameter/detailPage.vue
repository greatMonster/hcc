...
<script>
import { detailList } from '@/apiNew/securityChannel'
export default {
  name: 'Details',
  filters: {
  ...
  methods: {
    editData() {
      ...
      this.$router.push({ path: '/dispatching/securityChannel/addPage', query: { id: this.id, businessDomainId: this.$route.query.businessDomainId } })
    },
    initable() {
    ...
    linktoTraffics(data) {
      this.$router.push({ path: '/dispatching/securityChannel/detailsPage', query: { id: data.id } })
    }
  }
}
</script>
