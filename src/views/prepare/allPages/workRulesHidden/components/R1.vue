<template>
  <div class="rule1-content">
    <div class="rule1-content-left">
      ...
      <div class="rule1-content-left-bottom" style="margin-top:10px">
        <div class="rule1-content-left-bottom-input">
          <el-input v-model="data.adRules.common[0].costtext" placeholder="请输入内容 CNY" clearable />
        </div>
        <div class="rule1-content-left-bottom-select">
          <el-select v-model="data.adRules.common[0].per" clearable placeholder="请选择">
            <el-option
              v-for="selectChild in options"
              :key="selectChild.value"
              :label="selectChild.label"
              :value="selectChild.label"
            />
          </el-select>
        </div>
      </div>
    ...
    <div class="rule1-content-right">
      ...
      <div v-for="(item,index1) in data.adRules.cover" :key="index1" class="rule1-content-right-middle">
        ...
        <div class="rule1-content-right-middle-right">
          ...
          <div class="rule1-content-left-bottom-select rule1-content-right-bottom-select">
            <el-select v-model="item.choice" clearable placeholder="请选择">
              <el-option
                v-for="optionChild in options1"
                :key="optionChild.value"
                :label="optionChild.label"
                :value="optionChild.value"
              />
            </el-select>
          </div>
        </div>
        <el-button v-if="item.showCancel=='true'" class="showCancel" @click="cancelRule(index1)"> <i class="el-icon-delete" /> 删除</el-button>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      data: { adRules: { common: [{ costtext: '', per: '' }],
        cover: [{
          add: '',
          checkedDay: '',
          checkedDayarry: [],
  ...
  created() {
    if (!this.parent.adRules.cover) { // 如果adrules为空,给个初始值为空
      ...
      this.parent.adRules.cover.forEach(element => {
...
.rule1-content-left,.rule2-content-left {
  width: 25%;
  height: 120px;
  color: white;
}
