<template>
  <div>
      ...
      <div class="content-box">
        ...
        <div class="content-left">
          ...
          <div class="doc">
            ...
            <div class="doc-content-box">
              <div class="doc-content1">
                <span>航班计划</span>
                <el-select v-model="FLIGHT_SCHEDULE" placeholder="请选择" class="particle-button" style="width:150px" size="small" @change="flightChange">
                  <el-option v-for="item in flightoptions" :key="item.documentId" :label="item.name" :value="item.documentId" />
                </el-select>
              </div>
              <div class="doc-content2">
                <span>服务标准</span>
                <el-select v-model="rulesetDocumentId" placeholder="请选择" style="width:150px" size="small">
                  <el-option v-for="item in workRulesDocOptions" :key="item.documentId" :label="item.name" :value="item.documentId" />
                </el-select>
              </div>
              <div class="doc-content2">
                工作量名称
                <el-input v-model="shiftPlanName" placeholder="请输入工作量名称" style="width:150px" size="small" />
              </div>
            </div>
          </div>
          <!-- 周期 -->
          <div class="recycle">
            ...
            <div class="recycle-content-box">
              <div class="recycle-content1">
                <span>从日期</span>
                <span>
                  <el-date-picker v-model="fromDate" type="date" placeholder="选择日期" format="yyyy-MM-dd:HH:mm:ss" class="recycle-button" size="small" />
                </span>
              </div>
              <div class="recycle-content2">
                <span>到日期</span>
                <span>
                  <el-date-picker v-model="toDate" type="date" placeholder="选择日期" format="yyyy-MM-dd:HH:mm:ss" class="recycle-button" size="small" />
                </span>
              </div>
            </div>
          </div>
          ...
          <div class="particle">
            ...
            <div class="particle-content-box">
              <div class="particle-content1">
                <span>计算</span>
                <span>
                  <el-select v-model="particle01" disabled placeholder="请选择" class="particle-button" size="small">
              ...
              <div class="particle-content2">
                <span>结果</span>
                <span>
                  <el-select v-model="granularityInterval" placeholder="请选择" class="particle-button" size="small">
              ...
              <div class="particle-content2">
                <span>抽样模式</span>
                <span>
                  <el-select v-model="workloadSampling" placeholder="请选择" class="particle-button" size="small">
          ...
          <div style="text-align: center;width:40%;margin: 14px auto 0;line-height:40px;height:120px;">
            ...
            <el-button v-if="documentId" type="primary" size="mini" @click="mergeWorkload">合并工作量</el-button>
            <el-button v-if="documentId" type="primary" size="mini" @click="splitWorkload">拆分工作量</el-button>
          </div>
        </div>
        <!-- 图表展示 -->
              ...
              <div class="workload">
                <div class="workload-top">
                  ...
                  <div class="span-value">
                    <el-date-picker v-model="workloadDate" value-format="yyyy-MM-dd" type="date" placeholder="选择日期" default-value size="small" />
                  </div>
                  <div class="span-value">开始 为期</div>
                  <div class="span-value">
                    <el-date-picker v-model="workloadDate2" type="date" placeholder="选择日期" value-format="yyyy-MM-dd" default-value size="small" />
                  </div>
                  <div class="span-value">的工作量</div>
                  <el-button size="small" @click="search2">查询</el-button>
                </div>
    ...
    <!-- 拆分工作量弹框 -->
    <el-dialog v-dialogDrag title="拆分工作量" :visible.sync="dialogTableVisibleSplit" width="38%">
      <splitWorkload :document-id="documentId" />
    </el-dialog>
  </div>
</template>
...
import { workloadsetting, searchDate, tabhb, generateWorkload, flightDate } from '@/api/workload'
import LineChart from '@/components/echarts/LineChart'
import kpiComponent from './components/kpiComponent'
import splitWorkload from './components/splitWorkload'
export default {
  name: 'WorkloadHidden',
  components: { kpiComponent, LineChart, LineChartBig, splitWorkload },
  data() {
    return {
  ...
  methods: {
    jump() {
      this.$router.push({ path: '/prepare/workloadHiddenNew' })
    },
    flightChange(e) {
      this.startTime = ''
      this.endTime = ''
      ...
      flightDate(data).then(response => {
        if (response.code === '200') {
          this.fromDate = response.data.from.split(' ')[0]
          this.toDate = response.data.to.split(' ')[0]
          this.startTime = response.data.from.split(' ')[0]
          this.endTime = response.data.to.split(' ')[0]
        } else {
          this.$message({
            type: 'error',
