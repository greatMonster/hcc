    ...
    <div class="search">
      ...
      <el-card class="box-card">
        ...
        <div class="seach-content">
          ...
          <div class="text item">2. 上传的文件一共三列，均为必填。第一列需是每个时刻的日期，相邻时间需按照5分钟粒度间隔，第二列需是技能名,技能名需要在系统内存在，第三列需是当前时刻所需技能数，技能数为整数。</div>
          <div class="text item">
            <span>3.&nbsp;</span>
            <selected-import class="filter-top-item right-section2" :name="name" :post-url="url" :headers-data="headersData" @refreshData="initTable" />
          </div>
        </div>
      </el-card>
...
export default {
  ...
  data() {
    return {
      ...
      name: 'file',
      url: `${process.env.VUE_APP_BASE_API}prepare/api/v1/workloads/import?businessDomainId=${this.$route.query.businessDomainId || ''}&airportId=${this.$store.state.user.airport.airportId}`,
      errorShow: false
    }
  },
  created() {
    console.log(axios, 132132)
  },
  methods: {
    stepClick(index) {
      this.active = index
    ...
    viewClick() {
      axios.get(`${process.env.VUE_APP_BASE_API}` + 'prepare/api/v1/workloads/importTemplate', {
