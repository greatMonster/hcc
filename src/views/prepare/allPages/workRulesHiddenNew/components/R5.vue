    ...
    <div class="edit-box">
      <div v-for="(item,index) in data.parameterBean.extendsParam" :key="index" class="rthirty-five-time">
        <div class="rthirty-five-box3">
          轮班应开始于
          <el-time-select v-model="item.startTime" style="width:120px" placeholder="起始时间" :picker-options="{ start: '00:00', step: '00:15', end: '24:00' }" />到
          <el-time-select v-model="item.endTime" style="width:120px" placeholder="结束时间" :picker-options="{ start: '00:00',step: '00:15', end: '24:00',minTime: item.startTime}" />时间内
          <div>
            <el-checkbox-group v-model="item.checkedDayarry" style="display:inline-flex; vertical-align: middle;" @change="handleChange(item,dayindex)">
              <el-checkbox-button v-for="(day,dayindex) in days" :key="dayindex" :label="day">{{ day }}</el-checkbox-button>
            </el-checkbox-group>
            <el-button v-if="item.showCancel=='true'" class="showCancel" @click="cancelRule(index)">
