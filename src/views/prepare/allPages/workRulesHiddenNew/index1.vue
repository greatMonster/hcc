<template>
  <div class="skillBox">
    ...
    <div class="content" style="padding-bottom: 100px;">
      <el-tabs v-model="editableTabsValue" type="card" editable @edit="handleTabsEdit" @tab-click="tabhideClick">
        <el-tab-pane v-for="items in editableTabs" :key="items.id" :label="items.name" :name="items.id.toString()">
          <div>
    ...
    <div>
      <el-dialog v-dialogDrag title="新增" :visible.sync="landDialogVisible" width="40%" :before-close="cancelAddLand">
        <el-form ref="formEdit" :model="formEdit" label-width="100px">
          <el-form-item prop="landCode" label="员工类型">
            <el-select v-model="formEdit.Id" clearable filterable multiple placeholder="请选择员工类型">
              <el-option v-for="(items, indexs) in Options" :key="indexs" :label="items.name" :value="items.id" />
            </el-select>
          </el-form-item>
        </el-form>
        <div slot="footer" class="dialog-footer">
          <el-button style="background: #495373;color: white;" size="small" @click="cancelAddLand">取 消</el-button>
          <el-button type="primary" size="small" @click="handleLandEditClick('formEdit')">确 定</el-button>
        </div>
      </el-dialog>
    </div>
    <!-- 规则组dialog -->
    <div>
      <el-dialog v-dialogDrag title="可添加规则组" :visible.sync="dialogVisible" width="40%" :before-close="cancel">
...
export default {
  ...
  methods: {
    initData() {
      initrulseData(this.$route.query.id).then(response => {
        this.editableTabs = response.data.data.employeeTypes
      ...
      if (action === 'add') {
        if (!this.editableTabs.length) {
          ...
          for (let i = 0; i < this.editableTabs.length; i++) {
            this.arr.push(this.editableTabs[i].id)
          }
          for (let j = 0; j < this.Options.length; j++) {
            for (let i = 0; i < this.arr.length; i++) {
              if (this.Options[j].id == this.arr[i]) {
                this.rulesOptions.push(this.Options[j])
                this.Options.splice(j, 1)
              }
            }
          }
    ...
    addSkillClick() {
      if (this.editableTabs.length) {
        ...
        this.$message.error('请添加员工类型')
      }
    },
    handleCopyClick() {
      if (this.formEdit.copyId) {
        var obj3 = []
        this.editableTabs.forEach(item => {
          if (item.id == this.formEdit.copyId) {
            obj3 = JSON.parse(JSON.stringify(item.rules))
          }
          if (item.id == this.editableTabsValue) {
            item.rules = JSON.parse(JSON.stringify(obj3))
            item.rules.forEach(itm => {
              if (itm.parameterBean.extendsParam) {
                itm.parameterBean.extendsParam.forEach(element => {
                  element.checkedDayarry = JSON.stringify(element.checkedDayarry)
                })
              }
            })
          }
    ...
    EditClick() {
      for (let i = 0; i < this.editableTabs.length; i++) {
        if (this.isLandId) {
          if (this.isLandId == this.editableTabs[i].id) {
            this.editableTabs[i].rules = []
            this.editableTabs[i].rules = this.rulesitem1.ruleTemplates
            this.editableTabs[i].groupId = this.rulesitem1.id
          }
        } else {
    ...
    // 员工类型
    select() {
      selectData(this.$route.query.businessDomain).then(response => {
        this.Options = response.data.data
      })
    },
    tabhideClick(targetName) {
      this.editableTabs.forEach(item => {
        if (item.id == targetName.name) {
          this.nameData = item.groupName
        }
      })
    ...
    handleConfirmClick: _debounce(function(_type, index, item) {
      if (this.editableTabs.length) {
        if (this.$route.query.offon || this.$route.query.type === '复制') {
          ...
          initrulse(this.workTypeData).then(response => {
          ...
          this.workTypeData.employeeTypes = this.editableTabs
          this.workTypeData.businessDomain = this.$route.query.businessDomain
          editData(this.workTypeData).then(response => {
            if (response.data.code == '200') {
