<template>
  <div style="margin-top:60px">
    <div class="add-user">
      <el-form :inline="true" :model="listQuery" size="small">
        <el-form-item label="承运人">
          <el-select v-model="listQuery.carrier" placeholder="请选择" style="width:220px" clearable>
        ...
        <el-form-item label="航班号">
          <el-select v-model="listQuery.flightNo" filterable clearable style="width:220px">
            <el-option v-for="item in flightNoArry" :key="item.val" :label="item.val" :value="item.val" />
          </el-select>
        </el-form-item>
        <el-form-item label="机型">
          <el-select v-model="listQuery.acType" filterable clearable style="width:220px">
            <el-option v-for="item in AirportArry" :key="item.val" :label="item.val" :value="item.val" />
          </el-select>
        </el-form-item>
        <el-form-item>
    ...
    <div class="detail-style">
      <span>已生成关联航班{{ hasRelation }}</span>
      <span>，未处理航班{{ noRelation }}个</span>
    </div>
    <div class="table-css">
      <el-table :data="tableData" border stripe>
        ...
        <el-table-column prop="intlFlag" label="国际/国内">
          <template slot-scope="scope">
            <span v-if="scope.row.intlFlag=='D'">
              国际
            </span>
            <span v-if="scope.row.intlFlag=='I'">
              国内
            </span>
          </template>
        </el-table-column>
        <el-table-column prop="flightNoJ" label="进港航班号" />
        <el-table-column prop="flightNoC" label="出港航班号" />
        <el-table-column prop="actype" label="机型" />
        <el-table-column prop="weekDay" label="班期" />
        <el-table-column prop="throughFlag" label="是否过站">
          <template slot-scope="scope">
            <span v-if="scope.row.throughFlag=='0'">
              否
            </span>
            <span v-if="scope.row.throughFlag=='1'">
              是
            </span>
          </template>
        </el-table-column>
