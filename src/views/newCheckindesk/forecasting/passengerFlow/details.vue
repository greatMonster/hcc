    ...
    <div class="box">
      <el-card class="box-card">
          ...
          <el-row :gutter="20">
            ...
            <el-col :span="6">
              <div class="grid-content bg-purple" style="text-align: right;">
                <el-button type="primary" size="small" @click="editData()">编 辑</el-button>
              </div>
            </el-col>
          </el-row>
        </div>
        <div class="content">
          ...
          <div>
            <p class="leftP">以上条件对应离港航班数 ：</p>
            <p class="rightP">{{ formEdit.flightNum }}</p>
          </div>
        </div>
      </el-card>
      <el-card class="box-card">
        ...
        <div class="content">
          ...
          <div>
            <p>自助值机率 ：</p>
            <span>{{ formEdit.selfServiceRate }}%</span>
          </div>
          <div>
            <p>自助值机后柜台托运行李占比：</p>
            <span>{{ formEdit.luggageRate }}%</span>
          </div>
        </div>
      </el-card>
      <el-card class="box-card">
...
import { initData2, previewData, carriersData } from '@/api/passengerFlow'
export default {
  components: {
    LineChart,
    LineChartBig
  ...
  methods: {
    ...
    editData() {
      this.$router.push({ path: '/newcheckIn/operationPage', query: { id: this.id, offon: 'edit' }})
    },
    handleChange() {},
    closeImport() {
      // 关闭显示大图
    ...
    statusbtnClick() {
      ...
      previewData(this.formEdit).then((response) => {
    ...
    previewBig() {
      ...
      //         element.flightSeatFlowDetails.name = '离港座位'
      //         element.flightSeatFlowDetails.kpiName = '座位总数：' + element.flightSeatFlowDetails.total + '个'
      //         element.trafficDetails.name = '旅客流量'
      //         element.trafficDetails.kpiName = '旅客总数：' + element.trafficDetails.total + '个'
      //         var dataChart = [element.flightFlowDetails, element.flightSeatFlowDetails, element.trafficDetails]
      //         dataChart.forEach(element2 => {
      //           var data2 = {}
