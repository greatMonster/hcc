    ...
    <el-form ref="formEdit" :rules="rules" :model="formEdit" :label-width="formLabelWidth">
      <el-card v-for="(item, index) in datas" :key="index" class="box-card">
        ...
        <el-form-item v-if="item.passRate" :label="item.passRate" prop="passRate">
          <el-input v-model="formEdit.passRate" placeholder clearable type="number" />
        </el-form-item>
        <el-form-item v-if="item.maxQueuingTime" :label="item.maxQueuingTime" prop="maxQueuingTime">
          <el-input v-model="formEdit.maxQueuingTime" placeholder clearable type="number" />
        </el-form-item>
        ...
        <el-form-item v-if="item.employeeSkill" prop="employeeSkillId">
          <div slot="label" style="display: inline-block">
            技能
            <el-tooltip class="item" content="这里指定本次预测中，值机任务所需的员工技能" placement="bottom">
...
<script>
import { airports, employeeSkills, detailList } from '@/apiNew/checkInData'
export default {
  name: 'StepOne',
  data() {
    ...
    return {
      ...
      datas: [
        ...
        {
          title: '柜台服务标准',
          minWorkHours: '最短任务时间（小时）',
          maxWorkHours: '最长任务时间（小时）',
      ...
      rules: {
        ...
        maxWorkHours: [{ validator: maxWorkHoursValidate, trigger: 'blur', required: true }],
        minRestInterval: [{ required: true, message: '请输入最小休息间隔' }],
        passRate: [{ required: true, message: '请输入', trigger: 'blur' }],
        maxQueuingTime: [
          { required: true, message: '请输入', trigger: 'blur' },
          {
            validator: function (rule, value, callback) {
              if (/^[0-9]+([.]{1}[0-9]{1,2})?$/.test(value) === false) {
                callback(new Error('只是正数且能有2位小数'))
              } else {
                callback()
              }
            },
        ...
        minDeskOpenTime: [{ required: true, message: '请输入', trigger: 'blur' }],
  ...
  methods: {
    ...
    detailList() {
      if (localStorage.getItem('edit_stepOne')) {
        ...
        // console.log(this.formEdit, 1122)
      } else {
        detailList(this.routerId).then((response) => {
