<template>
  <div v-loading="loading">
    ...
    >
      ...
      <elx-editable-column prop="sex" label="性别" :edit-render="{name: 'ElSelect', options: sexList}" />
      <elx-editable-column prop="age" label="年龄" :edit-render="{name: 'ElInputNumber', props: {min: 1, max: 200}}" />
      <elx-editable-column prop="region" label="地区" :edit-render="{name: 'ElCascader', props: {options: regionList}}" />
      <elx-editable-column prop="date" label="日期" :edit-render="{name: 'ElDatePicker', props: {type: 'datetime', format: 'yyyy-MM-dd'}}" />
      <elx-editable-column prop="rate" label="评分" :edit-render="{name: 'ElRate', type: 'visible'}" />
      ...
      <elx-editable-column prop="attr3" label="属性3" :edit-render="{name: 'ElInput'}" />
      <elx-editable-column prop="attr4" label="属性4" :edit-render="{name: 'ElInput'}" />
      <elx-editable-column prop="attr5" label="属性5" :edit-render="{name: 'ElInput'}" />
...
export default {
  ...
  methods: {
    ...
    pendingRemoveEvent() {
      const selection = this.$refs.elxEditable.getSelecteds()
      if (selection.length) {
        const plus = []
        const minus = []
    ...
    deleteSelectedEvent() {
      const removeRecords = this.$refs.elxEditable.getSelecteds()
    ...
    submitEvent() {
      this.$refs.elxEditable.validate(valid => {
        if (valid) {
          const removeRecords = this.pendingRemoveList
          const { insertRecords, updateRecords } = this.$refs.elxEditable.getAllRecords()
