<template>
  <div class="gantt-timeline" :style="{'margin-left':-cellWidth/2+'px'}">
    <div v-for="(day,index) in getDays" :key="index" class="gantt-timeline-block" :style="{width:getTimeScales(day).length*cellWidth+'px'}">
      <div class="gantt-timeline-day " :style="heightStyle">
        {{ day.format("MM/DD") }}</div>
      <div class="gantt-timeline-scale " :style="heightStyle">
        <div v-for="(hour,index) in getTimeScales(day)" :key="index" :style="cellWidthStyle">
          {{ hour }}
        </div>
      </div>
    </div>
