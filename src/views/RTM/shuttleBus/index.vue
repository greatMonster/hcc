<template>
  <div id="app2">
    <div style="color:#fff;background: #3e4660; padding:8px 0;font-size:14px;margin-top:60px">
      ...
      <el-button size="small" @click="openStandard">派工标准</el-button>
      <span class="textips">
        航班数 :
        <ins>{{ getStat.flightNo }}</ins>，当前已分配任务 :<ins>{{ getStat.taskHasFenpei }}</ins>，当前未分配任务 : <ins>{{ getStat.taskNoFenpei }}</ins>，
        自动派工率 :<ins>{{ getStat.percentage }}</ins>，
        已接收任务 :<ins>{{ getStat.taskHasFenpei }}</ins>，未接收任务 :
        <ins>{{ getStat.taskHasFenpei }}</ins>，
        <span @click="drawer = true">告警数量 :<ins>{{ getStat.alarmNo }}10</ins>，</span>
        已处理告警 :<ins>{{ getStat.alarmHasHandle }}</ins>，
        未处理告警 :<ins>{{ getStat.alarmNoHandle }}</ins>
      </span>
      <el-drawer
        title="告警数量:10"
        :visible.sync="drawer"
    ...
    <div class="container" @dragend="dragend($event)" @dragover="allowDrop($event)" @drop="onDrop($event)">
      ...
      >
        <template v-slot:block="{data,getWidthAbout2Times,getPositonOffset,isInRenderingTimeRange,startTimeOfRenderArea,endTimeOfRenderArea}">
          <div v-for="(item,index) in data.taskMap" :id="index" :key="index" :employeeId="data.employeeId" :skillId="data.skillId" class="taskList" :h="index" :style="{top:3*index+'px'}">
            ...
            >
              <Test :data="data" :cell-height="cellHeight" :update-time-lines="updateTimeLines" :current-time="currentTime" :item="itemin" />
            ...
            <div class="staffTime" :style="{left:getPositonOffset(data.startTime)+'px',width:getWidthAbout2Times(data.startTime,data.endTime,{ scale,cellWidth})+'px'}" />
          </div>
        </template>
      ...
      <div class="leftbotm" :style="{'width':titleWidth+'px'}">
        <p>
          <el-button class="item-btn" size="small" type="success" @click="automaticTask">开始自动派工</el-button>
        </p>
        <p>
          <el-button class="item-btn" size="small" type="success" @click="stopBusTask">停止自动派工</el-button>
        </p>
        <p>
          <el-button class="item-btn" size="small" type="success" @click="showdialogFormake">新建临时任务</el-button>
        </p>
        <p><el-button class="item-btn" size="small" type="success" @click="cancelAssignAll">取消已派任务</el-button></p>
      </div>
      <!-- 修改规则 -->
...
export default {
  ...
  data() {
    return {
      ...
      timeLines: [
        {
          time: dayjs().subtract(dayjs().diff(dayjs('2019-1-15'), 'day'), 'day')
        },
        {
  ...
  methods: {
    ...
    stopBusTask() { // 停止自动派工
      removeBusTask().then(response => {
        this.init()
      })
    },
    getTaskState() { // 停止
      getTaskState().then(response => {
        if (response.code === '200') {
          this.stopstaus = response.data
        }
      })
    },
    cancelAssignAll() { // 取消已派任务
      var data = {
    ...
    opentask() { // 任务视图
      var routeData = this.$router.resolve({ path: '/dispatching/taskView', query: { name: '' }})
      window.open(routeData.href, '_blank')
    },
    openStandard() { // 派工标准
      this.$router.push({ path: '/dispatching/standard' })
    },
    handleEditClick() { // 规则弹框确定
    ...
    assignedTask() { // 员工列表
      ...
      this.timer = setInterval(this.assignedTask, 20000)
    },
    unassignedTask() { // 未分配
      clearInterval(this.timer)
      var data = {
    ...
    onDrop(event) {
      ...
      allotTasks(data).then(response => { // 任务分配
        if (response.code === '200') {
...
.container >>> .gantt-leftbar,.container >>> .leftbotm,
.container >>> .gantt-leftbar-wrapper {
  width: 200px!important;
  background: #000;
}
.container >>> .gantt-markline-area,.container >>> .gantt-scroll-x{
