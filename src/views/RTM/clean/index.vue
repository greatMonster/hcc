  ...
  <div id="app2">
    ...
    <div class="container" @dragend="dragend($event)" @dragover="allowDrop($event)" @drop="onDrop($event)">
      ...
      <div class="leftbotm" :style="{'width':titleWidth+'px'}">
        ...
        <p v-show="stopstaus">
          <el-button v-show="stopstaus==1" class="item-btn" size="small" type="success" @click="restart()">停止任务</el-button>
          <el-button v-show="stopstaus==0" class="item-btn" size="small" type="success" @click="restart()">重启任务</el-button>
        </p>
      </div>
      <!-- 修改规则 -->
...
export default {
  ...
  methods: {
    restart() {
      ...
      console.log(this.stopstaus, 88)
      if (this.stopstaus === '0') {
        this.stopstaus = '1'
      } else if (this.stopstaus === '1') {
        this.stopstaus = '0'
      }
    },
    getTaskState() { // 停止
    ...
    listFn() {
      ...
      this.timer = setInterval(this.listFn, 180000)
    },
    initList() {
      // 未分配
      var data = {
    ...
    updateTimeLines(timeA, timeB) {
      console.log(timeA, timeB, 88787878)
      this.timeLines = [
        {
          time: timeA
        },
