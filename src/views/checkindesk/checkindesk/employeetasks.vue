  ...
  <div id="app2">
    <div style="color: #fff; background: #3e4660; padding: 8px 0; font-size: 14px; padding-top: 60px">
      ...
      <el-form ref="ruleForm" :model="ruleForm" label-width="100px" class="demo-ruleForm" size="small">
        <label>参数组合：</label>
        <el-select v-model="intlType1" style="width: 320px" multiple collapse-tags size="small" placeholder="请选择">
        ...
        <el-button size="small" @click="jumpDesk">柜台视角</el-button>
      </el-form>

      <span class="textips">
        当日出港航班数 :
        <ins>{{ statsVO.flightNum }}</ins>，总任务数 : <ins>{{ statsVO.totalTaskNum }}</ins>， <span style="display: inline-block; width: 10px; height: 10px; background: #3a8ee6; margin-right: 3px" />未分配 : <ins>{{ statsVO.unassignedNum }}</ins>， <span style="display: inline-block; width: 10px; height: 10px; background: #1156f0; margin-right: 3px" />已分配 : <ins>{{ statsVO.assignedNum }}</ins>， <span style="display: inline-block; width: 10px; height: 10px; background: rgb(103, 194, 58) 50%; margin-right: 3px" />已开始&nbsp;&nbsp;: <ins>{{ statsVO.startedNum }}</ins>， <span style="display: inline-block; width: 10px; height: 10px; background: rgb(67, 162, 43) 50%; margin-right: 3px" />已接收 : <ins>{{ statsVO.receivedNum }}</ins>， <span style="display: inline-block; width: 10px; height: 10px; background: #a4abc0; margin-right: 3px" />已完成 : <ins>{{ statsVO.completedNum }}</ins>， <span style="display: inline-block; width: 10px; height: 10px; background: #99004d; margin-right: 3px" />已取消 : <ins>{{ statsVO.canceledNum }}</ins>， <span style="display: inline-block; width: 10px; height: 10px; background: #ff3399; margin-right: 3px" />已超时 : <ins>{{ statsVO.timeoutNum }}</ins>， <span style="color: yellow">！</span>告警 : <ins>{{ statsVO.warningNum }}</ins>， <span style="color: #67c13a"> <i class="el-icon-lock" /> </span>锁定 : <ins>{{ statsVO.assignedLockedNum + statsVO.unassignedLockedNum }}</ins>， 总工时利用率 :
    ...
    <div class="container" @dragend="dragend($event)" @dragover="allowDrop($event)" @drop="onDrop($event)" ref="gantt">
      ...
      >
        <template v-slot:block="{ data, getWidthAbout2Times, getPositonOffset, isInRenderingTimeRange, startTimeOfRenderArea, endTimeOfRenderArea }">
          <div :employeeId="data.employeeCode" :employeeName="data.employeeName" :skillId="data.skillId" class="taskList" :style="{ top: 3 + 'px' }">
            <div
              v-for="(item, index) in data.checkInTaskBeans"
              :id="index"
              :key="index"
            ...
            >
              <Test :data="data" :cell-height="cellHeight" :current-time="currentTime" :item="item" :status-list="statusList" @changeStatus="changeStatus" />
      ...
      >
        <template v-slot:block="{ data, getWidthAbout2Times, getPositonOffset, isInRenderingTimeRange, startTimeOfRenderArea, endTimeOfRenderArea }">
          <div v-if="data.checkInTaskBeans.length < 1" :employeeId="1" :employeeName="1" style="height: 100%; padding-top: 11px">无任务</div>
          <div
            v-for="(item2, index) in data.checkInTaskBeans"
            v-else
            :key="index"
            :employeeId="1"
            :employeeName="1"
          ...
          >
            <div style="position: relative">
              <div @dragstart="dragstart($event, item2)" @mouseenter="changeActive($event, item2)" @mouseleave="changeLeave($event)">
                <Test :data="data" :cell-height="cellHeight" :current-time="currentTime" :item="item2" :status-list="statusList" @changeStatus="changeStatus" />
              </div>
            </div>
          </div>
        </template>
...
import { ByEmployeeGroup1, noPersonTask1, deskManualDispatching, deskManualDispatching1, deskManualDispatching2, canceltask, dragSearch } from '@/api/checkInData'
import { searchTaskStatus } from '@/apiNew/vehicle'
import { parameterList } from '@/api/checkInNewHome'
import { deskChart1 } from '@/api/checkInData'
...
export default {
  ...
  methods: {
    ...
    jumpDesk() {
      this.$router.push({ path: '/dispatching/checkIn/deskTasks', query: { siteId: this.$route.query.siteId || 10, date: this.ruleForm.date, deskPredictions: this.intlType1.join(',') || '', businessDomainId: this.businessDomainId }})
    },
    intlTypeList() {
      this.regions1 = []
      const data = {
    ...
    listFn() {
      ...
      ByEmployeeGroup1(data)
        .then((response) => {
          ...
          } else if (data.employeeVOList.length === 0) {
            this.datasB = []
            this.positionB = { x: this.$route.query.data ? localStorage.scrollx - 600 : 3200, y: 10 }
    ...
    dragstart(ev, itemin) {
      ...
      this.employeeCodeV = itemin.employeeCode
      this.employeeName11 = itemin.employeeName
      //   console.log(itemin, 110)
      this.employeeSearch(itemin.taskId, itemin.employeeCode, itemin.startTime, itemin.endTime)
    },
    employeeSearch(taskId, employeeCode, startTime, endTime) {
      if (!employeeCode) {
        ...
        for (let i = 0; i < this.datasBCopy.length; i++) {
          let flag = true
          const employeeStartTime = new Date(this.datasBCopy[i].startTime).getTime()
          const employeeEndTime = new Date(this.datasBCopy[i].endTime).getTime() + 30 * 60 * 1000
          if (taskEndTime > employeeEndTime || taskStartTime < employeeStartTime) {
            flag = false
            continue
          } else {
            for (let j = 0; j < this.datasBCopy[i].checkInTaskBeans.length; j++) {
              // 如果冲突则删除该人并跳出循环
              if (!(taskEndTime <= new Date(this.datasBCopy[i].checkInTaskBeans[j].startTime).getTime() || taskStartTime >= new Date(this.datasBCopy[i].checkInTaskBeans[j].endTime).getTime())) {
    ...
    onDrop(event) {
      // console.log(event.target.childNodes[0].nodeName, 5522)
      // console.log(this.employeeCodeV, 5566)
      var data1 = {}
      if (event.target.getAttribute('employeeId') === '1' || (event.target.childNodes[0].nodeName !== '#text' && event.target.childNodes[0].getAttribute('employeeId') === '1')) {
        data1 = {
          employeeId: '',
