<template>
  <div class="warp desk-wrap">
    ...
    <div class="tips">
      ...
      <label>柜台服务参数：</label>
      <el-select v-model="intlType" size="small" style="min-width: 320px" multiple collapse-tags placeholder="请选择" @change="change2">
      ...
      <el-button size="small" @click="linktodesk">柜台视角</el-button>
    ...
    <div v-loading="loading" style="min-height: 200px" element-loading-text="计算中..." element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.8)">
      <ul v-if="list && list.length > 0" style="height: 200px">
        <li v-for="(item, index) in list" :key="index">
          <div class="left">
            <span>{{ item.isLandName || '经 理' }}</span>
          </div>
          <div class="right">
    ...
    <el-dialog v-dialogDrag :visible.sync="dialogdispathVisible" title="定时派工">
      <el-form ref="form" :model="form" :rules="formrules">
        ...
        <el-form-item prop="params" label="柜台服务参数" :label-width="formLabelWidth">
...
import { deskChart1, recalculation1, stopRecalculation1, recalculationStatus, dispachData, dispachFcn, preCalc, addCalc, taskGenerate, taskAssignmention } from '@/api/checkInData'
...
export default {
  ...
  data() {
    return {
      ...
      dataList: {},
      formrules: {
        params: [{ required: true, message: '请选择定时派工参数', trigger: ['change', 'blur'] }]
      },
      // optionsSiteId: [],
      siteId: '',
      remark: '',
  ...
  methods: {
    ...
    jumpTaskList() {
      var routeData = this.$router.resolve({ path: '/dispatching/checkIn/deskTaskList', query: { date: this.date, businessDomainId: this.businessDomainId } })
    ...
    dialogAddbtn() {
      ...
      this.form.airportId = this.airport.airportId
      this.form.businessDomainId = this.businessDomainId
      ...
      initAirpot(data)
        .then((response) => {
          if (response.data.data) {
            ...
            const storeSiteId = Number(this.$store.state.checkinParams.siteId)
            if (storeSiteId) {
    ...
    siteChange(e) {
      ...
      this.list = []
      this.siteId = e
      this.$store.commit('deleteDeskPredictions', null)
      this.intlTypeList()
    },
    getTime() {
      // setInterval(() => {
    ...
    init() {
      ...
      if (typeof WebSocket === 'undefined') {
        ...
        if (document.location.hostname === 'localhost') {
          ...
          wsuri = `ws://prod.rqm.yldev.net` + '/rtm/api/v1/webSock/' + data.id + '?airportId=' + data.airportId + '&deskPredictions=' + data.deskPredictions + '&date=' + data.date
        } else {
    ...
    getMessage: function (msg) {
      ...
      if (JSON.parse(msg.data).code === '200') {
        ...
        this.list = data1.checkInIsLandBeans
        ...
        if (!data1.checkInIsLandBeans) {
          // this.$message.success('暂无数据')
          this.noData = true
        } else {
          this.noData = false
          if (data1.checkInIsLandBeans) {
            data1.checkInIsLandBeans.forEach((element) => {
              element.checkInDeskBeans.forEach((element2) => {
                ...
                if (!element2.checkInTaskBeans.length && element2.openType === '0') {
                  element2.openTimes.forEach((item) => {
                    element2.personLessList.push({ startTime: item.beginTime, endTime: item.endTime, dutyPerson: '无人' })
                    element2.personlessDeskclassF = true
                    element2.deskclass = false
                    cont++
                  })
                  element2.bean.planStarTime = element2.openTimes[0].beginTime
                  element2.bean.planEndTime = element2.openTimes[element2.openTimes.length - 1].endTime
                }
                element2.checkInTaskBeans.forEach((element3) => {
                  ...
                  if (element3.employeeName == null) {
                    ...
                    element2.personLessList.push({ startTime: element3.startTime, endTime: element3.endTime, dutyPerson: element3.employeeName })
                  }
                  if (now > time && now < closeTime) {
                    aside = 1
                    if (element3.employeeName) {
                      openNumb1 += 1
                      element2.bean.employeeGroupName = element3.employeeName
                      this.checkinEmployee.push(element3.employeeName)
                    } else {
                      personless = 1
    ...
    send: function () {
      ...
      this.socket.send(this.intlType[0] ? JSON.stringify(data) : '{airportId:' + this.airportId.airportId + ',date:' + this.date + ',deskPredictions:null}')
      // this.socket.send()
    },
    close: function (e) {
    ...
    linktotask() {
      if (this.intlType.length) {
        var routeData = this.$router.resolve({ path: '/dispatching/checkIn/employeetasks', query: { siteId: this.airport.airportId || 10, date: this.date, deskPredictions: this.intlType.join(',') || '', businessDomainId: this.businessDomainId } })
        window.open(routeData.href, '_blank')
        // this.$router.push({ path: '/dispatching/checkIn/employeetasks', query: { siteId: this.siteId || 10, date: this.date, deskPredictions: this.intlType || '' } })
      } else {
    ...
    linktodesk() {
      if (this.intlType) {
        var routeData = this.$router.resolve({ path: '/dispatching/checkIn/deskTasks', query: { siteId: this.airport.airportId || 10, date: this.date, deskPredictions: this.intlType.join(',') || '', businessDomainId: this.businessDomainId } })
        window.open(routeData.href, '_blank')
        // this.$router.push({ path: '/dispatching/checkIn/deskTasks', query: { siteId: this.siteId || 10, date: this.date, deskPredictions: this.intlType || '' } })
      } else {
        this.$message.warning('请先选择组合参数')
      }
    },
