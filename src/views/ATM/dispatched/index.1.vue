...
export default {
  ...
  mounted () {
    ...
    $.get('https://www.echartsjs.com/examples/data/asset/data/airport-schedule.json', function (rawData) {
      console.log(rawData)
  ...
  methods: {
    ...
    renderGanttItem (params, api) {
      ...
      this._cartesianXBounds[0] = coordSys.x
      this._cartesianXBounds[1] = coordSys.x + coordSys.width
      this._cartesianYBounds[0] = coordSys.y
      this._cartesianYBounds[1] = coordSys.y + coordSys.height
    ...
    initDrag () {
      this._autoDataZoomAnimator = makeAnimator(dispatchDataZoom)
      ...
      this.myChart.getZr().on('mouseup', function () {
        ...
        if (this._draggingEl && this._dropRecord) {
          updateRawData() && this.myChart.setOption({
            series: {
              id: 'flightData',
              data: this._rawData.flight.data
            }
      ...
      updateRawData() {
        var flightData = this._rawData.flight.data
      ...
      autoDataZoomWhenDraggingOutside(cursorX, cursorY) {
        ...
        var cursorDistX = getCursorCartesianDist(cursorX, this._cartesianXBounds)
        var cursorDistY = getCursorCartesianDist(cursorY, this._cartesianYBounds)
