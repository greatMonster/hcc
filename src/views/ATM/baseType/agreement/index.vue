  ...
  <div class="skillBox">
    ...
    <div class="skillShow">
      <el-table border stripe :data="contractShowTable" @current-change="handleSelectChange">
        ...
        <el-table-column prop="contractTypeName" label="名称" />
        <el-table-column prop="contractTypeDesc" label="描述" />
        <el-table-column sortable prop="contractTypeCode" label="编码">
          <template slot-scope="scope">{{ scope.row.contractTypeCode=='FULL' ? '全职' : '兼职' }}</template>
        </el-table-column>
    ...
    <div>
      <el-dialog v-dialogDrag :title="title" :visible.sync="editSkillVisible">
        <el-form ref="formName" :model="formEdit" :rules="rules">
          <el-form-item prop="contractTypeName" label="名称：" :label-width="formLabelWidth">
            <el-input v-model="formEdit.contractTypeName" placeholder="请输入" />
          </el-form-item>
          <el-form-item prop="contractTypeDesc" label="描述：" :label-width="formLabelWidth">
            <el-input v-model="formEdit.contractTypeDesc" placeholder="请输入" />
          </el-form-item>
          <el-form-item prop="contractTypeCode" label="编码：" :label-width="formLabelWidth">
            <!-- <el-input v-model="formEdit.contractTypeCode" placeholder="请输入" /> -->
            <el-select v-model="formEdit.contractTypeCode" clearable filterable placeholder="请选择" style="width:140px">
              <el-option v-for="item in codeOptions" :key="item.code" :label="item.name" :value="item.code" />
            </el-select>
          </el-form-item>
        </el-form>
...
<script>
import { initTable2, add, remove, set } from '@/api/basegreement'
export default {
  name: 'BaseAgreement',
  data() {
    return {
      ...
      contractShowTable: [],
      formEdit: { contractTypeName: '', contractTypeDesc: '', contractTypeCode: '', contractTypeId: '' },
      rules: {
  ...
  methods: {
    ...
    // 添加合同
    addContractClick() {
      this.formEdit.contractTypeName = ''
      this.formEdit.contractTypeDesc = ''
      this.formEdit.contractTypeCode = ''
      this.title = '添加'
      this.editSkillVisible = true
    },
    // 修改行数据按钮
    editHandleClick(index, row) {
      this.formEdit.contractTypeId = row.contractTypeId
      this.title = '修改'
      this.formEdit = Object.assign({}, row) // copy obj
      this.editSkillVisible = true
    },
    edituserbleClick() {},
    // 修改行数据按钮的确定按钮
