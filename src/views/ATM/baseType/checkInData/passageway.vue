  ...
  <div class="skillBox">
    <div class="title">
      <div class="titleWord">
        ...
        <span style="padding-left: 20px;">
          <el-button type="primary" size="small" @click="search('search')">查 询</el-button>
        </span>
      </div>
    </div>
    <div class="content">
      <el-tabs v-model="editableTabsValue" type="card" @tab-click="paneClick">
        <el-tab-pane v-for="item in editableTabs" :key="item.securityDomainId" v-loading="bloading" :label="item.securityDomainName" :name="item.securityDomainId+''">
          <div class="addButton">
            <span>{{ item.securityDomainName }} 共 {{ passagewayData?passagewayData.length:'0' }}通道</span>
            <el-button type="primary" size="small" @click="addSkillClick('formEdit')">新增通道</el-button>
          </div>
          <el-table :data="passagewayData" border stripe :max-height="tableHeight" row-key="id" class="aaa">
            <el-table-column prop="code" label="编号" min-width="100" />
            <el-table-column prop="serviceTypeName" label="名称" min-width="400">
...
import { inintData, addDesk, editDesk, deleteData, Dragfcn, selectData } from '@/apiNew/passageway'
  ...
  methods: {
      ...
      tbody.forEach((item, i) => {
        Sortable.create(item, {
          onEnd({ newIndex, oldIndex }) {
            const currRow = _this.passagewayData.splice(oldIndex, 1)[0]
            _this.passagewayData.splice(newIndex, 0, currRow)
            var data = {
              id: currRow.id,
              weight: newIndex
            }
    ...
    search(e) {
      if (e) {
        ...
        initTable(data).then(res => {
          ...
          this.editableTabsValue = this.editableTabs[0].securityDomainId + ''
          data.securityDomainId = this.editableTabsValue
          this.landName = this.editableTabs[0].securityDomainName
          inintData(data).then(res => {
            if (res.data.code === '200') {
              if (res.data.data) {
                this.passagewayData = res.data.data
        ...
        initTable(data2).then(res => {
          ...
          data2.securityDomainId = this.editableTabsValue
          inintData(data2).then(res => {
            if (res.data.code === '200') {
              if (res.data.data) {
                this.passagewayData = res.data.data
              }
    ...
    paneClick(targetName) {
      ...
      var data = {
        terminalId: this.siteId,
        airportId: this.airport.airportId,
        securityDomainId: targetName.name
      }
      inintData(data).then(res => {
        this.passagewayData = []
        if (res.data.code === '200') {
    ...
    addSkillClick(formName) {
      ...
      this.formEdit = { landName: this.landName, airportId: this.airport.airportId, terminalId: this.siteId, securityDomainId: this.editableTabsValue }
    ...
    editHandleClick(index, row) {
      ...
      if (row.openType === 0) {
        ...
        if (row.openTimes) {
          if (row.openTimes[0]) {
            this.initTime = [row.openTimes[0].beginTime, row.openTimes[0].endTime]
            if (row.openTimes.length > 1) {
              for (let i = 1; i < row.openTimes.length; i++) {
                this.times.push({ time: [row.openTimes[i].beginTime, row.openTimes[i].endTime] })
              }
            }
          }
        }
        ...
        if (valid) {
          ...
          if (this.title === '修改') {
            ...
            addDesk(this.formEdit)
              .then(response => {
                if (response.data.code === '200') {
                  this.editableTabsValue = '' + response.data.data.securityDomainId
                  this.search()
                  this.$message({
                    type: 'success',
                    message: response.data.message
