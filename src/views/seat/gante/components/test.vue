<template>
  ...
  >
    ...
    <div
      ...
      slot="reference"
      class="plan"
      :style="{'background':statusUnAssigned,'box-shadow':statusUnAssigned2}"
      @click="onClick"
    >
      <div class="runTime">
        <!-- <span>S:{{startToString}}</span> -->
    ...
    <div class="detail">
      <span class="header">航班号:{{ item.name }}</span>
      <ul>
        <li>
          <span>上班时间：</span><span>{{ startToString }}</span>
        </li>
        <li>
          <span>上班时间：</span><span>{{ endToString }}</span>
        </li>
      </ul>
    </div>
  </el-popover>
...
const NOW_PLAN2 = 'rgb(5, 127, 5) 9px 15px 33px -8px inset, rgb(156, 232, 145) 0px 0px 0px, rgb(6, 80, 39) 0px 0px 0px'
const FUTHER_PLAN2 = 'rgb(10, 54, 234) 9px 15px 33px -8px inset, rgb(101, 126, 232) 0px 0px 0px, rgb(35, 54, 165) 0px 0px 0px'
const PAST_PLAN2 = 'rgb(129, 138, 167) 0px 1px 10px 1px inset, rgb(167, 167, 167) 0px 0px 0px, rgb(62, 70, 96) 0px 0px 0px'
export default {
  name: 'Test',
  props: {
    data: Object,
  ...
  data() {
    return {
      dayjs: dayjs,
      statusUnAssigned: 'linear-gradient(rgb(135, 191, 249) 50%, #2d93fd 50%, rgb(1, 163, 255) 100%)',
      statusUnAssigned2: 'rgb(10, 54, 234) 9px 15px 33px -8px inset, rgb(101, 126, 232) 0px 0px 0px, rgb(35, 54, 165) 0px 0px 0px'
    }
  },
  computed: {
    statusColor() {
