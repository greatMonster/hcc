<template>
  <div class="warp">
    <div class="title">
      <el-form ref="ruleForm" :model="ruleForm" label-width="60px" class="demo-ruleForm" size="small">
        <el-form-item label="日期：" prop="region">
          <el-date-picker v-model="ruleForm.data" type="date" placeholder="选择日期" value-format="yyyy-MM-dd" />
        </el-form-item>
        <el-form-item label="航班：" prop="flightNo">
          <!-- <el-input v-model="ruleForm.flightNo" placeholder="请输入航班号" class="input-with-select" /> -->
          <el-select v-model="ruleForm.flightNo" filterable placeholder="请选择">
          ...
          <!-- <el-button type="primary" size="mini" @click="exportClick">导 出</el-button> -->
        </el-form-item>
      </el-form>
    ...
    <el-table v-loading="listLoading" element-loading-text="加载中..." :max-height="tableHeight" element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.5)" stripe :data="tableData">
      <el-table-column type="index" width="45" label />
      <el-table-column prop="flightNo" label="航班号" width="80" />
      <el-table-column prop="positionType" label="机位" width="100" />
      ...
      <el-table-column prop="cabinLayout" label="人数(经济/高端)" width="80" />
      <el-table-column prop="taskNum" label="任务数" width="50">
        <template slot-scope="scope">{{ scope.row.tasks.length }}</template>
      </el-table-column>
          ...
          <div class="list">
            <div v-for="(item, index) in scope.row.tasks" :key="index" class="divlist" :data="index">
              <el-popover trigger="hover" placement="top">
                <p>任务: {{ item.name }}</p>
                <p>计划开始时间：{{ item.planInPlace | joinDate }}</p>
                <p>计划结束时间：{{ item.planEnd | joinDate }}</p>
                <p>实际开始时间：{{ item.actualInPlace | joinDate }}</p>
                <p>实际结束时间：{{ item.actualEnd | joinDate }}</p>
...
<script>
import { getFlightPlanInfo, getFlightstats, addTemporaryTaskData } from '@/apiNew/vehicle'
export default {
  name: 'TaskViewNew',
  filters: {
  ...
  data() {
    return {
      ...
      options: [],
      ruleForm: {
        data: this.$route.query.date ? this.$route.query.date : this.getNowFormatDate(),
  ...
  methods: {
    // exportClick() {
    //   window.location.href = `${process.env.VUE_APP_BASE_API}` + `rtm/api/v1/tasks/ferryBus/flightAndTask?date=${this.ruleForm.data || ''}`
    // },
    getFlightNo() {
      addTemporaryTaskData({ flightNo: '', date: this.date }).then((response) => {
    ...
    init() {
      ...
      getFlightPlanInfo(this.ruleForm).then((response) => {
        // init
    ...
    getFlightstatsFn() {
      getFlightstats(this.ruleForm).then((response) => {
        // init
        if (response.data.data) {
    ...
    openNext(item) {
      ...
      const routeData = this.$router.resolve({ path: '/vehicle/shuttleBus/dispatching', query: { id: item.id, skillId: item.skillId, data: data, left: localStorage.scrollx } })
    ...
    pickdta(e) {
      if (e == null) {
        ...
        this.ruleForm.data = this.getNowFormatDate()
        this.init()
      } else {
