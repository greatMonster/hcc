...
export function taskListInit(data) {
  return httpRequest({
    url: '/rtm/api/v1/desks/tasks/list?beginDate=' + data.beginDate + '&endDate=' + data.endDate + '&businessDomainId=' + data.businessDomainId + '&airportId=' + data.airportId,
    method: 'get',
    data
  })
}
...
export function queryList(data) {
  return request({
    url: '/adm/CheckInAllocationBaseData/queryList?siteId=' + data.siteId,
    method: 'get'
  })
}
...
export function distribution1(data) { // 新的柜台视角查询
  return httpRequest({
    url: '/rtm/api/v1/desks/deskView?airportId=' + data.airportId + '&deskPredictions=' + data.deskPredictions + '&employeeName=' + data.employeeName + '&date=' + data.date,
    method: 'get',
    data
  })
}
...
export function ByEmployeeGroup1(data) {
  return httpRequest({
    url: 'rtm/api/v1/desks/employeeView?airportId=' + data.airportId + '&deskPredictions=' + data.deskPredictions + '&employeeName=' + data.employeeName + '&date=' + data.date,
    method: 'get'
  })
}
// 员工视图为匹配到人的任务请求
...
export function noPersonTask1(data) {
  return httpRequest({
    url: 'rtm/api/v1/desks/task?airportId=' + data.airportId + '&deskPredictions=' + data.deskPredictions + '&intlType=' + data.intlType + '&employeeName=' + data.employeeName + '&date=' + data.date,
    method: 'get'
  })
}
// 员工视图拖拽
...
export function deskManualDispatching1(data) {
  return httpRequest({
    url: '/rtm/api/v1/desks/employeeView/change?taskId=' + data.checkInTaskId + '&employeeCode=' + data.employeeId + '&date=' + data.date,
    method: 'put',
    data
  })
}
...
export function canceltask(data) {
  return httpRequest({
    url: '/rtm/api/v1/desks/cancel/' + data.taskId,
    method: 'put',
    data
  })
}
...
export function dispachData(data) {
  return httpRequest({
    url: '/rtm/api/v1/desks/assignment/scheduled/' + data.airportId + '/' + data.businessDomainId,
    method: 'get'
  })
}
...
export function boardGate(data) {
  return httpRequest({
    url: 'rtm/api/v1/boardGate/boardGateView?taskTypes=' + data.deskPredictions + '&date=' + data.date,
    method: 'get'
  })
}
// 任务拆分
